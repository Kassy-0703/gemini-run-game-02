...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetGameState() {
        ...
        state = {
            player: createPlayer(groundY), obstacles: [], items: [], visualEffects: [],
            ...
            currentScreen: 'title', groundY: groundY,
        };
    }
    ...
    function startGame() {
        ...
        gameInterval = setInterval(() => {
            ...
            state.timer--;
            if (state.timer <= 0) startGameOverSequence();
        }, 1000);
        gameLoop();
    }
    ...
    function createPlayer(groundY) {
        return {
            ...
            draw() {
                ...
                if (assets.player) ctx.drawImage(assets.player, this.x, this.y, this.width, this.height);
                ctx.globalAlpha = 1.0;
            },
            update() {
    ...
    function manageSpawning() {
        ...
        if (--state.itemSpawnCooldown <= 0) {
            ...
            state.itemSpawnCooldown = getRandomInt(200, 350);
        }
    }
    ...
    function createFallingObstacle() {
        state.obstacles.push({
            x: getRandomInt(CONFIG.GAME_WIDTH, CONFIG.GAME_WIDTH * 1.5), y: -30, width: 37.5, height: 37.5, vy: 4,
    ...
    function createFastObstacle() {
        state.obstacles.push({
            x: CONFIG.GAME_WIDTH, y: state.groundY - 45, width: 75, height: 45, vx: -4,
    ...
    function activateItem(type, x, y) {
        ...
        switch(type) {
            ...
            case 'invincible':
                ...
                createInvincibleEffect(state.player.x + state.player.width / 2, state.player.y + state.player.height / 2);
                break;
            case 'speedUp':
    ...
    function createInvincibleEffect(x, y) {
        state.visualEffects.push({
            life: 60, x, y,
            update() { this.radius = 50 * (1 - this.life / 60); },
            draw() {
                ctx.globalAlpha = this.life / 60;
                if (assets.itemInvincibleEffect) {
                    const s = this.radius * 2;
                    ctx.drawImage(assets.itemInvincibleEffect, this.x - s / 2, this.y - s / 2, s, s);
                }
            }
        });
    }
    ...
    function switchScreen(screenName) {
        ...
        // ゲーム画面のときだけスクロールを���止し、それ以外（タイトル等）では許可する
        if (screenName === 'game') {
    ...
    function setupEventListeners() {
        ...
        dom.buttons.jump.addEventListener('click', (e) => {
            e.stopPropagation();
            handleInput(e);
        });
